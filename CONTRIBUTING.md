...
- P09 - When the default behavior of an action is undesirable in a context, override it to throw an exception. This has the advantage of stopping execution, which prevents the calling code from continuing with incorrect assumptions. You may additionally provide a notification.
- P10 - Do not implement an action in a context where it is irrelevant if it has no default implementation because Talon handles this case for us.