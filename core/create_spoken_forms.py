...
from .numbers.numbers_definitions import Number, digits, digits_map, scales, teens, tens
...
def create_spoken_form_for_number(num: int):
    ...
    for i, x in enumerate(n3):
        ...
        if b2 == 0:
            if type(ones[b1] is Number):
                words = [ones[b1].spoken_forms[0], t] + words
            else:
                words = ["", t] + words
        elif b2 == 1:
            if type(teens[b1] is Number):
                words = [teens[b1].spoken_forms[0], t] + words
            else:
                words = ["", t] + words
        elif b2 > 1:
            first = twenties[b2]
            second = ones[b1]
            if type(first is Number):
                first = first.spoken_forms[0]
            if type(second is Number):
                second = second.spoken_forms[0]

            words = [first, second, t] + words
        if b3 > 0:
            value = ones[b3]
            if type(ones[b3] is Number):
                value = ones[b3].spoken_forms[0]

            words = [value, scales[0].spoken_forms[0]] + words

    # filter out the empty strings and join
    return " ".join(filter(None, words))


print(create_spoken_form_for_number(1023))
print(create_spoken_form_for_number(1000023))
print(create_spoken_form_for_number(1270023))
