...
def get_context_page(index: int) -> int:
    return math.ceil(index / settings.get("user.help_max_command_lines_per_page"))


def get_total_context_pages() -> int:
    return math.ceil(
        len(sorted_display_list) / settings.get("user.help_max_command_lines_per_page")
    )


def get_current_context_page_length() -> int:
    start_index = (current_context_page - 1) * settings.get("user.help_max_command_lines_per_page")
    return len(
        sorted_display_list[
            start_index : start_index + settings.get("user.help_max_command_lines_per_page")
        ]
    )
...
def paginate_list(data, SIZE=None):
    chunk_size = SIZE or settings.get("user.help_max_contexts_per_page")
    it = iter(data)
...
class Actions:
    ...
    def help_select_index(index: int):
        ...
        if gui_context_help.showing:
            if index < settings.get("user.help_max_command_lines_per_page") and (
                (current_context_page - 1) * settings.get("user.help_max_command_lines_per_page")
                + index
                ...
                if selected_context is None:
                    selected_context = display_name_to_context_name_map[
                        sorted_display_list[
                            (current_context_page - 1)
                            * settings.get("user.help_max_command_lines_per_page")
                            + index
                        ]
                    ]
