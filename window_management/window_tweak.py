...
tag = mod.tag(
    TAG_NAME, desc="Enable stop command during continuous window move/resize."
)
...
def _win_show_gui(gui: imgui.GUI) -> None:
    ...
    gui.text(
        f"Center: {round(screen.visible_rect.center.x), round(screen.visible_rect.center.y)}"
    )
    gui.spacer()
...
class WinCompassControl:
    @classmethod
    def win_set_rect(
        cls, old_rect: ui.Rect, rect_id: int, rect_in: ui.Rect
    ) -> tuple[bool, ui.Rect]:
        ...
        while retries >= 0:
            ...
            if event_count == 0:
                ...
                if testing:
                    print(
                        "_win_set_rect: nothing to do, window already matches given rect."
                    )
            ...
            try:
                ...
                position_matches_request = (rect_in.x, rect_in.y) == (
                    w.rect.x,
                    w.rect.y,
                )
                size_matches_request = (rect_in.width, rect_in.height) == (
                    w.rect.width,
                    w.rect.height,
                )
                if not position_matches_request or not size_matches_request:
                    if False and app.platform == "linux":
                        if testing:
                            print(
                                "_win_set_rect: linux - timed out waiting for window update."
                            )

                        if retries > 0:
                            if testing:
                                print(
                                    "_win_set_rect: linux - retrying after time out..."
                                )
                            retries -= 1
                            continue
                        else:
                        ...
                        raise compass_control.RectUpdateError(
                            rect_id=rect_id,
                            initial=old_rect,
                            requested=rect_in,
                            actual=w.rect,
...
class Actions:
    ...
    def win_move(direction: Optional[Direction] = None) -> None:
        ...
        compass_control.mover.continuous_init(
            w.rect,
            w.id,
            w.screen.visible_rect,
            w.screen.dpi_x,
            w.screen.dpi_y,
            direction,
        )
    ...
    def win_move_absolute(
        x: float, y: float, region: Optional[Direction] = None
    ) -> None:
    ...
    def win_move_to_pointer(
        region: Optional[NonDualDirection] = non_dual_direction(["north", "west"])
    ):
        "Move window to pointer position, centered on the point indicated by the given region"

        w = ui.active_window()

        compass_control.mover.resize_to_pointer(
            w.rect, w.id, w.screen.visible_rect, region
        )
    ...
    def win_stretch(direction: Optional[Direction] = None) -> None:
        ...
        w = ui.active_window()
        compass_control.sizer.continuous_init(
            w.rect,
            w.id,
            w.screen.visible_rect,
            1,
    ...
    def win_resize_absolute(
        target_width: float, target_height: float, region: Optional[Direction] = None
    ) -> None:
        ...
        compass_control.sizer.resize_absolute(
            w.rect, w.id, target_width, target_height, region
        )
    ...
    def win_resize_to_pointer(nd_direction: NonDualDirection) -> None:
        ...
        compass_control.sizer.resize_to_pointer(
            w.rect, w.id, w.screen.visible_rect, nd_direction
        )
    ...
    def win_move_pixels(distance: int, direction: Optional[Direction] = None) -> None:
        ...
        delta_width, delta_height = compass_control.get_component_dimensions(
            w.rect, w.id, w.screen.visible_rect, distance, direction, "move"
        )

        return compass_control.mover.move_pixels_relative(
            w.rect, w.id, w.screen.visible_rect, delta_width, delta_height, direction
        )
    ...
    def win_move_percent(percent: float, direction: Optional[Direction] = None) -> None:
        ...
        delta_width, delta_height = compass_control.get_component_dimensions_by_percent(
            w.rect, w.id, w.screen.visible_rect, percent, direction, "move"
        )
    ...
    def win_resize_pixels(distance: int, direction: Optional[Direction] = None) -> None:
        ...
        delta_width, delta_height = compass_control.get_component_dimensions(
            w.rect, w.id, w.screen.visible_rect, distance, direction, "resize"
        )

        if testing:
            print(f"win_resize_pixels: {delta_width=}, {delta_height=}")

        compass_control.sizer.resize_pixels_relative(
            w.rect, w.id, w.screen.visible_rect, delta_width, delta_height, direction
        )

    def win_resize_percent(
        percent: float, direction: Optional[Direction] = None
    ) -> None:
        ...
        delta_width, delta_height = compass_control.get_component_dimensions_by_percent(
            w.rect, w.id, w.screen.visible_rect, percent, direction, "resize"
        )
