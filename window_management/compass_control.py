...
from talon import Context, Module, cron, ctrl, settings, ui
...
class CompassControl:
    def __init__(
        ...
        self.mover: CompassControl.Mover = CompassControl.Mover(
            self, settings_map, testing
        )
        self.sizer: CompassControl.Sizer = CompassControl.Sizer(
            self, settings_map, testing
        )
    ...
    def _update_all_settings(cls, caller, caller_id: str) -> None:
        ...
        for local_name, talon_setting in caller.settings_map.items():
            if hasattr(caller, local_name):
                ...
                print(
                    f"{caller_id}._update_all_settings: received updated value for {talon_setting.path}: {getattr(caller, local_name, None)}"
                )
    ...
    def _update_setting(cls, caller, caller_id: str, args):
        ...
        try:
            ...
            print(
                f"{caller_id}._update_setting: received updated value for {talon_name}: {getattr(caller, local_name, None)}"
            )
    ...
    def _handle_rect_update_error(self, e: RectUpdateError) -> None:
        if self.verbose_warnings:
            position_matches_request = (e.requested.x, e.requested.y) == (
                e.actual.x,
                e.actual.y,
            )
            size_matches_request = (e.requested.width, e.requested.height) == (
                e.actual.width,
                e.actual.height,
            )
            ...
            if not position_matches_request:
                logging.warning(
                    f"after update, rectangle position does not exactly match request: {e.requested.x, e.requested.y} -> {e.actual.x, e.actual.y}"
                )

            if not size_matches_request:
                logging.warning(
                    f"after update, rectangle size does not exactly match request: {e.requested.width, e.requested.height} -> {e.actual.width, e.actual.height}"
                )
    ...
    class Mover:
        ...
        def continuous_move_frequency_str(self):
            if self._continuous_move_frequency_str is None:
                self._continuous_move_frequency_str = self.settings_map[
                    "_continuous_move_frequency_str"
                ].get()
        ...
        def continuous_move_frequency(self):
            if self._continuous_move_frequency is None:
                self._continuous_move_frequency = float(
                    (self.continuous_move_frequency_str)[:-2]
                )
        ...
        def continuous_move_rate(self):
            if not self._continuous_move_rate is None:
                self._continuous_move_rate = self.settings_map[
                    "_continuous_move_rate"
                ].get()
        ...
        def continuous_init(
            ...
            with self.compass_control.continuous_mutex:
                if self.continuous_job:
                    ...
                    logging.warning(
                        "cannot start a move job when one is already running"
                    )
                    return
                ...
                (
                    self.continuous_width_increment,
                    self.continuous_height_increment,
                ) = self.compass_control._get_continuous_parameters(
                    rect,
                    rect_id,
                    parent_rect,
                    self.continuous_move_rate,
                    self.continuous_move_frequency,
                    dpi_x,
                    dpi_y,
                    direction,
                    "move",
                ...
                if self.testing:
                    print(
                        f"init_continuous: {self.continuous_width_increment=}, {self.continuous_height_increment=}"
                    )

                direction_count = sum(
                    self.compass_control.continuous_direction.values()
                )
                if (
                    direction_count == 0 or direction_count == 4
                ):  # move in/out (special case)
                    # follow path from rectangle center to the target point

                    x0 = round(rect.x)
                    y0 = round(rect.y)

                    x1, y1 = self.compass_control.get_target_point(
                        rect, rect_id, parent_rect, direction
                    )
                    ...
                    self.continuous_bres = self.compass_control.bresenham(
                        x0, y0, x1, y1
                    )
        ...
        def _continuous_start(self) -> None:
            ...
            with self.compass_control.continuous_mutex:
                ...
                self.continuous_job = cron.interval(
                    self.continuous_move_frequency_str, self._continuous_helper
                )
                if self.testing:
        ...
        def _continuous_helper(self) -> None:
            ...
            def _move_it(
                ...
                if not result:
                    if self.testing:
                        print(
                            f"continuous_helper: rectangle move failed. {result=}, {rect=}, {horizontal_limit_reached=}, {vertical_limit_reached=}"
                        )
                elif (
                    horizontal_limit_reached and vertical_limit_reached
                ):  # both limits reached, we are done
                    if self.testing:
                        print(
                            f"continuous_helper: rectangle move is complete. {result=}, {rect=}, {horizontal_limit_reached=}, {vertical_limit_reached=}"
                        )
                    self.continuous_width_increment = (
                        self.continuous_height_increment
                    ) = 0
            ...
            def _continuous_move_in_out(
                ...
                while (
                    ...
                    (
                        ...
                        and (
                            self.continuous_width_increment
                            or self.continuous_height_increment
                        )
                    )
                    or
                    ...
                    (
                        direction_count == 4
                        and self.continuous_width_increment
                        and self.continuous_height_increment
                    )
                ):
                    ...
                    if self.testing:
                        print(
                            f"continuous_helper: stepping from {rect.x, rect.y} to {x, y}, {delta_x=}, {delta_y=}"
                        )

                    # print(f'continuous_helper: before move {rect=}')
                    result, rect = _move_it(
                        rect,
                        rect_id,
                        parent_rect,
                        delta_x,
                    ...
                    if self.testing:
                        print(
                            f"continuous_helper: {cumulative_delta_x=}, {self.continuous_width_increment=}"
                        )
                    if (
                        self.continuous_width_increment != 0
                        and cumulative_delta_x >= abs(self.continuous_width_increment)
                    ):
                        if self.testing:
                            print(
                                f"continuous_helper: moved horizontally as far as possible for current iteration, stopping"
                            )
                        break
                    #
                    # vertical check
                    ...
                    if self.testing:
                        print(
                            f"continuous_helper: {cumulative_delta_y=}, {self.continuous_height_increment=}"
                        )
                    if (
                        self.continuous_height_increment != 0
                        and cumulative_delta_y >= abs(self.continuous_height_increment)
                    ):
                        if self.testing:
                            print(
                                f"continuous_helper: moved vertically as far as possible for current iteration, stopping"
                            )
                        break
            ...
            with self.compass_control.continuous_mutex:
                ...
                if self.testing:
                    print(
                        f"continuous_helper: iteration {iteration} mutex wait ({elapsed_time_ms} ms)"
                    )
                ...
                if self.testing:
                    print(
                        f"continuous_helper: starting iteration {iteration} - {rect=}"
                    )
                ...
                if (
                    ...
                    if direction_count == 1 or direction_count == 2:
                        ...
                        # move in/out
                        rect = _continuous_move_in_out(
                            self, _move_it, rect, rect_id, parent_rect
                        )

                elapsed_time_ms = (time.time_ns() - start_time) / 1e6
                if self.testing:
                    print(
                        f"continuous_helper: iteration {iteration} done ({elapsed_time_ms} ms)"
                    )
                if elapsed_time_ms > self.continuous_move_frequency:
                    if self.compass_control.verbose_warnings != 0:
                        logging.warning(
                            f"continuous_helper: move iteration {iteration} took {elapsed_time_ms}ms, longer than the current move_frequency setting. actual rate may not match the continuous_rate setting."
                        )
        ...
        def move_pixels_relative(
            ...
            def _get_new_coordinates(
                ...
                if direction_count == 1 or direction_count == 2:
                    ...
                    if self.testing:
                        print(
                            f"move_pixels_relative: before clipping - new_x={x}, new_y={y}"
                        )
                    ...
                    if self.testing:
                        print(
                            f"move_pixels_relative: after clipping - {new_x=}, {new_y=}, {horizontal_limit_reached=}, {vertical_limit_reached=}"
                        )
                else:  # move in
                    new_x = x + delta_x
                    new_y = y + delta_y
                    ...
                    if self.testing:
                        print(
                            f"move_pixels_relative: {new_x=}, {new_y=}, {parent_rect.center.x=}, {parent_rect.center.y=}"
                        )
                        print(
                            f"move_pixels_relative: target point - {target_x=}, {target_y=}"
                        )
                    ...
                    if delta_x != 0:
                        ...
                        if (delta_x < 0 and (distance_x >= delta_x)) or (
                            delta_x > 0 and (distance_x <= delta_x)
                        ):
                            # crossed target point, done moving horizontally
                            if self.testing:
                                print(
                                    f"move_pixels_relative: crossed horizontal target point"
                                )
                            new_x = target_x
                    ...
                    if delta_y != 0:
                        ...
                        if (delta_y < 0 and (distance_y >= delta_y)) or (
                            delta_y > 0 and (distance_y <= delta_y)
                        ):
                            # crossed target point, done moving vertically
                            if self.testing:
                                print(
                                    f"move_pixels_relative: crossed vertical target point"
                                )
                            new_y = target_y
            ...
            (
                ...
                horizontal_limit_reached,
                vertical_limit_reached,
            ) = _get_new_coordinates(
                self, rect, rect_id, parent_rect, x, y, delta_x, delta_y, direction
            )
            ...
            if (new_x, new_y) == (rect.x, rect.y):
                ...
                try:
                    ...
                    if self.testing:
                        print(
                            f"move_pixels_relative: setting new rect: {rect} -> {new_rect}"
                        )

                    result, rect = self.compass_control.set_rect(
                        rect, rect_id, new_rect
                    )
        ...
        def move_absolute(
            ...
            x: float,
            y: float,
            region_in: Optional[Direction] = None,
        ) -> tuple[bool, ui.Rect]:
            ...
            try:
                result, rect = self.compass_control.set_rect(
                    rect,
                    rect_id,
                    ui.Rect(round(x), round(y), round(rect.width), round(rect.height)),
                )
            except CompassControl.RectUpdateError as e:
                self.compass_control._handle_rect_update_error(e)
        ...
        def resize_to_pointer(
            self, rect: ui.Rect, rect_id: int, parent_rect: ui.Rect, region: Direction
        ) -> tuple[bool, ui.Rect]:
            x, y = ctrl.mouse_pos()
            return self.move_absolute(rect, rect_id, round(x), round(y), region)

        def translate_top_left_by_region(
            self,
            rect: ui.Rect,
            rect_id: int,
            target_x: float,
            ...
            if self.testing:
                ...
                print(
                    f"_translate_top_left_by_region: move coordinates: {target_x=}, {target_y=}"
                )
            ...
            if self.testing:
                print(
                    f"_translate_top_left_by_region: translated position: {top_left_x=}, {top_left_y=}"
                )
        ...
        def _clip_to_fit(
            ...
            if x <= parent_x and (direction_count == 0 or direction["left"]):
                new_x = parent_x
                horizontal_limit_reached = True
            elif x >= parent_x + parent_width - width and (
                direction_count == 0 or direction["right"]
            ):
            ...
            if y <= parent_y and (direction_count == 0 or direction["up"]):
                new_y = parent_y
                vertical_limit_reached = True
            elif y >= parent_y + parent_height - height and (
                direction_count == 0 or direction["down"]
            ):
    ...
    class Sizer:
        def __init__(self, compass_control, settings_map: dict, testing: bool):
            ...
            self.use_change_check_for_shrink: bool = (
                not self.use_resize_history_for_shrink
            )
        ...
        def continuous_resize_frequency_str(self):
            if self._continuous_resize_frequency_str is None:
                self._continuous_resize_frequency_str = self.settings_map[
                    "_continuous_resize_frequency_str"
                ].get()
        ...
        def continuous_resize_frequency(self):
            if self._continuous_resize_frequency is None:
                self._continuous_resize_frequency = float(
                    (self.continuous_resize_frequency_str)[:-2]
                )
        ...
        def continuous_resize_rate(self):
            if not self._continuous_resize_rate is None:
                self._continuous_resize_rate = self.settings_map[
                    "_continuous_resize_rate"
                ].get()
        ...
        def continuous_init(
            ...
            with self.compass_control.continuous_mutex:
                if self.continuous_job:
                    logging.warning(
                        "cannot start a resize job when one is already running"
                    )
                    return
                ...
                (
                    ...
                    rect_id,
                    parent_rect,
                    self.continuous_resize_rate,
                    self.continuous_resize_frequency,
                    dpi_x,
                    dpi_y,
                    direction,
                    "_resize",
                ...
                if self.testing:
                    print(
                        f"init_continuous: starting resize - {self.continuous_width_increment=}, {self.continuous_height_increment=}, {self.compass_control.continuous_direction=}, {multiplier=}"
                    )
        ...
        def _continuous_start(self) -> None:
            ...
            with self.compass_control.continuous_mutex:
                ...
                # start the job
                self.continuous_job = cron.interval(
                    self.continuous_resize_frequency_str, self._continuous_helper
                )
        ...
        def _continuous_helper(self) -> None:
            ...
            def _check_history_for_max_shrinkage(self, rect: ui.Rect):
                ...
                if (
                    self.continuous_width_increment < 0
                    or self.continuous_height_increment < 0
                ):
                    ...
                    if len(self.continuous_resize_history) == 2:
                        if (
                            value
                            == self.continuous_resize_history[0]
                            == self.continuous_resize_history[1]
                        ):
                            # window size has stopped changing...so quit trying
                            if self.testing:
                                print(
                                    "_win_resize_continuous_helper: window size has stopped changing, quitting..."
                                )
                            self.compass_control.continuous_stop()
                        else:
            ...
            def _check_resize_limits(
                self,
                resize_left_limit_reached: bool,
                resize_up_limit_reached: bool,
                resize_right_limit_reached: bool,
                resize_down_limit_reached: bool,
            ) -> None:
                direction = self.compass_control.continuous_direction
                direction_count = sum(direction.values())
                if direction_count == 1:  # horizontal or vertical
                    if (
                        (resize_left_limit_reached and direction["left"])
                        or (resize_up_limit_reached and direction["up"])
                        or (resize_right_limit_reached and direction["right"])
                        or (resize_down_limit_reached and direction["down"])
                    ):
                        if self.testing:
                    ...
                    if (resize_left_limit_reached and direction["left"]) or (
                        ...
                        self.continuous_width_increment = 0
                    #
                    if (resize_up_limit_reached and direction["up"]) or (
            ...
            with self.compass_control.continuous_mutex:
                ...
                if (
                    not self.continuous_width_increment
                    and not self.continuous_height_increment
                ):
                    # if there's no work to do...
                    if self.testing:
                    ...
                    (
                        result,
                        rect,
                        resize_left_limit_reached,
                        resize_up_limit_reached,
                        resize_right_limit_reached,
                        resize_down_limit_reached,
                    ) = many_values

                    if self.testing:
                        print(
                            f"continuous_helper: resize returned - {result=}, {rect=}, {resize_left_limit_reached=}, {resize_up_limit_reached=}, {resize_right_limit_reached=}, {resize_down_limit_reached=}"
                        )
                    ...
                    if result:
                        ...
                        if rect and self.use_resize_history_for_shrink:
                            if self.testing:
                                print(
                                    f"continuous_helper: checking for max shrinkage...5"
                                )
                            self._check_history_for_max_shrinkage(rect)
                        else:
                            if self.testing:
                                print(
                                    f"continuous_helper: rectangle resize failed. {rect=}"
                                )
                            self.compass_control.continuous_stop()
            ...
            if elapsed_time_ms > self.continuous_resize_frequency:
                if self.compass_control.verbose_warnings != 0:
                    logging.warning(
                        f"continuous_helper: resize iteration {iteration} took {elapsed_time_ms}ms, longer than the current resize_frequency setting. actual rate may not match the current rate setting."
                    )
        ...
        def resize_pixels_relative(
            ...
            def _get_new_values(
                ...
                result = (
                    ...
                    resize_up_limit_reached
                ) = resize_right_limit_reached = resize_down_limit_reached = False
                ...
                if self.testing:
                    print(
                        f"resize_pixels_relative: starting {rect=}, {delta_width=}, {delta_height=}, {new_width=}, {new_height=}"
                    )
                ...
                if direction_count == 1:  # horizontal or vertical
                    ...
                    if direction["left"]:
                        ...
                        new_x, new_width, resize_left_limit_reached = self._clip_left(
                            rect, rect_id, parent_rect, new_x, new_width, direction_in
                        )
                    #
                    if direction["up"]:
                        new_y = new_y - delta_height
                        new_y, new_height, resize_up_limit_reached = self._clip_up(
                            rect, rect_id, parent_rect, new_y, new_height, direction_in
                        )
                    #
                    if direction["right"]:
                        new_x, new_width, resize_right_limit_reached = self._clip_right(
                            rect, rect_id, parent_rect, new_x, new_width, direction_in
                        )
                    #
                    if direction["down"]:
                        new_y, new_height, resize_down_limit_reached = self._clip_down(
                            rect, rect_id, parent_rect, new_y, new_height, direction_in
                        )
                    ...
                    direction_count == 0 or direction_count == 4
                ):  # stretch to/from center
                    # are delta values are divisible by two?
                    if (delta_width == 0 or abs(delta_width) >= 2) and (
                        delta_height == 0 or abs(delta_height) >= 2
                    ):
                        ...
                        if self.testing:
                            print(
                                f"resize_pixels_relative: delta width and/or height are too small (<2), alternating size and position changes"
                            )
                ...
                if self.testing:
                    ...
                    print(
                        f"resize_pixels_relative: {width=}, {new_width=}, {height=}, {new_height=}"
                    )
                    #
                return (
                    new_x,
            ...
            if (new_width, new_height) == (rect.width, rect.height):
                ...
                try:
                    # make it so
                    result, rect = self.compass_control.set_rect(
                        rect, rect_id, ui.Rect(*new_values)
                    )
                except CompassControl.RectUpdateError as e:
                    self.compass_control._handle_rect_update_error(e)
                ...
                if not result and self.use_change_check_for_shrink:
                    (
                        ...
                        resize_right_limit_reached,
                        resize_down_limit_reached,
                    ) = self._check_change_for_max_shrinkage(
                        rect, new_x, new_y, new_width, new_height, old_rect
                    )
        ...
        def resize_to_pointer(
            ...
            rect_id: int,
            parent_rect: ui.Rect,
            nd_direction: non_dual_direction,
        ) -> tuple[bool, ui.Rect, bool, bool, bool, bool]:
            if self.testing:
            ...
            if self.testing:
                print(
                    f"resize_to_pointer: {delta_width=}, {delta_height=}, {direction=}"
                )

            return self.resize_pixels_relative(
                rect, rect_id, parent_rect, delta_width, delta_height, direction
            )

        def _check_change_for_max_shrinkage(
            self,
            rect: ui.Rect,
            new_x: float,
            new_y: float,
            ...
            if self.continuous_width_increment < 0:
                ...
                if (new_x != old_rect.x and rect.x == old_rect.x) and (
                    new_width != old_rect.width and rect.width == old_rect.width
                ):
                    ...
                    if self.testing:
                        ...
                        print(
                            f"resize_pixels_relative: horizontal shrink limit reached - {rect.x=}, {new_x=}, {rect.width=}, {new_width=}"
                        )

            if self.continuous_height_increment < 0:
                # if a change was requested and not delivered, i.e. if the requested value is not the same as the old one AND
                # the requested value is not the same as the new value.
                if (new_y != old_rect.y and rect.y == old_rect.y) and (
                    new_height != old_rect.height and rect.height == old_rect.height
                ):
        ...
        def resize_absolute(
            ...
            if region_in:
                ...
                x, y = self.translate_top_left_by_region(
                    rect, rect_id, target_width, target_height, region
                )
            ...
            try:
                result, rect = self.compass_control.set_rect(
                    ...
                    rect_id,
                    ui.Rect(
                        round(x), round(y), round(target_width), round(target_height)
                    ),
                )
            except CompassControl.RectUpdateError as e:
                self.compass_control._handle_rect_update_error(e)
        ...
        def translate_top_left_by_region(
            ...
            if self.compass_control.verbose_warnings != 0:
                if abs(delta_width) < 2:
                    logging.warning(
                        f"_translate_top_left_by_region: width change is less than 2, which is too small for normal resize calculations, ymmv: {delta_width=}"
                    )
                if abs(delta_height) < 2:
                    logging.warning(
                        f"_translate_top_left_by_region: height change is less than 2, which is too small for normal resize calculations, ymmv: {delta_height=}"
                    )

            if self.testing:
                print(f"_translate_top_left_by_region: initial rect: {rect}")
                print(
                    f"_translate_top_left_by_region: resize coordinates: {target_width=}, {target_height=}"
                )
            ...
            if self.testing:
                print(
                    f"_translate_top_left_by_region: translated position: {x=}, {y=}, {target_width=}, {target_height=}"
                )
        ...
        def _clip_right(
            ...
            if x + width > parent_rect.x + parent_rect.width and (
                direction_count == 0 or direction["right"]
            ):
        ...
        def _clip_down(
            ...
            if y + height > parent_rect.y + parent_rect.height and (
                direction_count == 0 or direction["down"]
            ):
    ...
    def _get_continuous_parameters(
        ...
        if direction_count == 1:
            ...
            if (
                direction_count == 0 and operation == "move"
                ...
                direction_count == 4 and operation == "move"
        ...
        if self.testing:
            print(
                f"get_continuous_parameters: returning {width_increment=}, {height_increment=}"
            )
    ...
    def continuous_stop(self) -> None:
        ...
        with self.continuous_mutex:
            if not self.mover.continuous_job and not self.sizer.continuous_job:
                if self.testing:
                    print(
                        "continuous_stop: no jobs to stop (may have stopped automatically via clipping logic)"
                    )
                return
    ...
    def set_rect(
        self, old_rect: ui.Rect, rect_id: int, rect_in: ui.Rect
    ) -> tuple[bool, ui.Rect]:
    ...
    def _save_last_rect(self, rect_id=None, rect=None):
        ...
        self.last_rect = {"id": rect_id, "rect": rect}
    ...
    def snap(
        ...
        if (rect.x, rect.y) != (parent_rect.center.x, parent_rect.center.y):
            ...
            result, rect = self.mover.move_absolute(
                rect, rect_id, parent_rect.center.x, parent_rect.center.y, direction
            )
            if not result:
                if self.testing:
        ...
        if (rect.width, rect.height) != (parent_rect.width, parent_rect.height):
            # set rectangle size
            result, rect = self.sizer.resize_absolute(
                rect, rect_id, target_width, target_height, direction
            )
            if not result:
                if self.testing:
    ...
    def get_center_to_center_rect(
        self, rect: ui.Rect, rect_id: int, other_rect: ui.Rect
    ) -> tuple[ui.Rect, bool, bool]:
        ...
        center_to_center_rect = ui.Rect(
            round(other_center.x), round(rect_center.y), round(width), round(height)
        )
    ...
    def get_center_to_intercept_rect(
        self, rect: ui.Rect, rect_id: int, parent_rect: ui.Rect
    ) -> tuple[ui.Rect, bool, bool]:
        ...
        intercept_x, intercept_y = self.get_center_to_center_intercept(
            rect, parent_rect
        )
        ...
        result_rect = ui.Rect(
            round(intercept_x), round(rect_center.y), round(width), round(height)
        )
        if self.testing:
            print(
                f"get_center_to_intercept_rect: returning {result_rect=}, {horizontal_multiplier=}, {vertical_multiplier=}"
            )
        ...
        if operation == "move" and (
            ...
            if direction_count == 0:  # in
                (
                    ...
                    vertical_multiplier,
                ) = self.get_center_to_center_rect(rect, rect_id, parent_rect)
            else:  # direction_count == 4
                (
                    rect,
                    horizontal_multiplier,
                    vertical_multiplier,
                ) = self.get_center_to_intercept_rect(rect, rect_id, parent_rect)
            ...
            new_x = ((1 - ratio_of_differences) * rect_center.x) + (
                ratio_of_differences * parent_center.x
            )
            new_y = ((1 - ratio_of_differences) * rect_center.y) + (
                ratio_of_differences * parent_center.y
            )

            if self.testing:
                print(
                    f"get_component_dimensions: {diagonal_length=}, {new_x=}, {new_y=}"
                )
    ...
    def get_component_dimensions_by_percent(
        self,
        rect: ui.Rect,
        rect_id: int,
        parent_rect: ui.Rect,
        ...
        if operation == "move":
            if direction_count == 0:  # in
                rect, *unused = self.get_center_to_center_rect(
                    rect, rect_id, parent_rect
                )
            elif direction_count == 4:
                rect, *unused = self.get_center_to_intercept_rect(
                    rect, rect_id, parent_rect
                )
        ...
        return self.get_component_dimensions(
            rect, rect_id, parent_rect, distance, direction, operation
        )
    ...
    def get_edge_midpoint(
        self, rect: ui.Rect, direction: Direction
    ) -> tuple[float, float]:
    ...
    def get_center_to_center_intercept(self, rect, parent_rect) -> tuple[float, float]:
        ...
        if parent_rect.center.x == rect.center.x:
            ...
            if rect.center.y > parent_rect.center.y:
                ...
                print(
                    f"get_center_to_center_intercept: below - {result_x=}, {result_y=}"
                )
            else:
                result_y = parent_rect.y
                print(
                    f"get_center_to_center_intercept: above - {result_x=}, {result_y=}"
                )

            if self.testing:
                angle_between = math.radians(90)
                print(
                    f"get_center_to_center_intercept: {angle_between=} ({math.degrees(angle_between)} degrees), {result_x=}, {result_y=}"
                )
        ...
        slope2 = (rect.center.y - parent_rect.center.y) / (
            rect.center.x - parent_rect.center.x
        )
        ...
        if self.testing:
            print(
                f"get_center_to_center_intercept: {angle_between=} ({math.degrees(angle_between)} degrees), {radius_x=}, {radius_y=}, {result_x=}, {result_y=}"
            )

        return round(result_x), round(result_y)

    def get_target_point(
        self, rect: ui.Rect, rect_id: int, parent_rect: ui.Rect, direction: Direction
    ) -> tuple[int, int]:
        ...
        if direction_count == 0:  #  in/center
            ...
            (
                center_intercept_x,
                center_intercept_y,
            ) = self.get_center_to_center_intercept(rect, parent_rect)
            ...
            if self.testing:
                print(
                    f"get_target_point: center to center intercept - {center_intercept_x, center_intercept_y}"
                )
                print(
                    f"get_target_point: corresponding top left coordinates - {target_x, target_y}"
                )
