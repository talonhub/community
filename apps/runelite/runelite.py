...
@mod.action_class
class Actions:
    def toggle_key_hold(key_name: str):
        """Toggles holding down a specified key."""
        if key_name in hold_jobs:
            actions.user.stop_key_hold(key_name)
        else:
            actions.user.start_key_hold(key_name)

    def start_key_hold(key_name: str):
        ...
        if key_name not in hold_jobs:
            ...
            hold_jobs[key_name] = cron.interval("100ms", lambda: ctrl.key_press(key_name))

    def stop_key_hold(key_name: str):
