...
class Actions:
    ...
    def vscode_next_previous_helper(
        direction: str, command_id_prev: str, command_id_next: str
    ):
        """Execute command_id_prev or command_id_next via vscode command server
        depending on direction (PREV/NEXT)."""
        actions.user.vscode(command_id_prev if direction == "PREV" else command_id_next)
