...
This causes VS Code to include `lang:[python]` in the window title with the current active language ID. [Community's VS Code support looks for this string in the window title](vscode_language_mode.py#L14) to activate the correct `code.language`.

If you have existing customizations to your window title you want to keep, ensure that `lang:[${activeEditorLanguageId}]` appears somewhere within your custom `window.title`.
