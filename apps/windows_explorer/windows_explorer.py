...
if logger.level == logging.NOTSET:
    logger.setLevel(
        logging.ERROR
    )  # Set to DEBUG if you want to implement a new language mapping
...
def get_system_language():
    ...
    try:
        ...
        logger.debug(
            f"windows_explorer.get_system_language: {system_language}, code: {language_code}"
        )
        return language_code
    except KeyError:
        logger.error(
            "windows_explorer.get_system_language: Language detection failed, defaulting to English"
        )
        return "en"
...
def get_localized_folder_name(folder_id):
    ...
    try:
        ...
        logger.exception(
            f"windows_explorer.get_localized_folder_name: Exception getting localized name for {folder_id}: {e}"
        )
    ...
    try:
        ...
        for folder_key in STANDARD_FOLDERS:
            ...
            try:
                ...
                logger.exception(
                    f"windows_explorer.get_folder_names: API error for {folder_key}: {e}"
                )
        ...
        logger.exception(
            f"windows_explorer.get_folder_names: API approach failed, using English fallback: {e}"
        )
...
def get_folder_path(folder_key):
    ...
    if folder_id:
        try:
            ...
            logger.exception(
                f"windows_explorer.get_folder_path: SHGetKnownFolderPath failed for {folder_key}: {e}"
            )
    ...
    if not directories_to_remap:
        try:
            ...
            for folder_key in STANDARD_FOLDERS:
                ...
                logger.debug(
                    f"windows_explorer.initialize_directory_mappings: {localized_name} -> {actual_path}"
                )

            if user_display_name:
                mappings[user_display_name] = USER_PATH
                logger.debug(
                    f"windows_explorer.initialize_directory_mappings: User mapping {user_display_name} -> {USER_PATH}"
                )

            directories_to_remap = mappings
        except Exception as e:
            logger.exception(
                f"windows_explorer.initialize_directory_mappings: Failed to load directory mappings: {e}"
            )
            directories_to_remap = {}
...
if app.platform == "windows":
    ...
    directories_to_remap = {}
    current_language_mapping = LANGUAGE_MAPPINGS.get(
        language_code, LANGUAGE_MAPPINGS["en"]
    )
    logger.debug(
        f"windows_explorer: Windows platform init using language mapping for '{language_code}'"
    )
    ...
    if "current_language_mapping" in globals():
        for pattern in current_language_mapping["suffix_patterns"]:
            ...
            if match:
                ...
                logger.debug(
                    f"windows_explorer._strip_explorer_suffixes: Stripped suffix from '{path}'"
                )
                break
    return path
...
def _apply_directory_mappings(path):
    ...
    if path in directories_to_remap:
        ...
        logger.debug(
            f"windows_explorer._apply_directory_mappings: {path} -> {mapped_path}"
        )
        return mapped_path
    return path
...
def _handle_path_exclusions(path):
    ...
    """
    if (
        "current_language_mapping" in globals()
        and path in current_language_mapping["excludes"]
    ):
        logger.debug(
            f"windows_explorer._handle_path_exclusions: Directory excluded '{path}'"
        )
...
class UserActions:
    ...
    def file_manager_current_path():
        ...
        logger.debug(
            f"windows_explorer.UserActions.file_manager_current_path: Window title '{path}'"
        )
        ...
        logger.debug(
            f"windows_explorer.UserActions.file_manager_current_path: Final path '{path}'"
        )
        return path
