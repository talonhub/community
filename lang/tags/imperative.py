...
class Actions:
    ...
    def code_state_else_if():
        ...
        actions.user.deprecate_action("2025-06-24", "user.code_state_else_if", 'user.insert_snippet_by_name("elseIfStatement")')

    def code_state_else():
        """Inserts else statement"""
        actions.user.insert_snippet_by_name("elseStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_else", 'user.insert_snippet_by_name("elseStatement")')

    def code_state_do():
        """Inserts do statement"""
        actions.user.insert_snippet_by_name("doWhileLoopStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_do", 'user.insert_snippet_by_name("doWhileLoopStatement")')

    def code_state_switch():
        """Inserts switch statement"""
        actions.user.insert_snippet_by_name("switchStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_switch", 'user.insert_snippet_by_name("switchStatement")')

    def code_state_case():
        """Inserts case statement"""
        actions.user.insert_snippet_by_name("caseStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_case", 'user.insert_snippet_by_name("caseStatement")')

    def code_state_for():
        """Inserts for statement"""
        actions.user.insert_snippet_by_name("forLoopStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_for", 'user.insert_snippet_by_name("forLoopStatement")')

    def code_state_for_each():
        """Inserts for each equivalent statement"""
        actions.user.insert_snippet_by_name("forEachStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_for_each", 'user.insert_snippet_by_name("forEachStatement")')

    def code_state_go_to():
        """inserts go-to statement"""
        actions.user.insert_snippet_by_name("goToStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_go_to", 'user.insert_snippet_by_name("goToStatement")')

    def code_state_while():
        """Inserts while statement"""
        actions.user.insert_snippet_by_name("whileLoopStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_while", 'user.insert_snippet_by_name("whileLoopStatement")')

    def code_state_infinite_loop():
        """Inserts infinite loop statement"""
        actions.user.insert_snippet_by_name("infiniteLoopStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_state_infinite_loop", 'user.insert_snippet_by_name("infiniteLoopStatement")')
    ...
    def code_break():
        ...
        actions.user.deprecate_action("2025-06-24", "user.code_break", 'user.insert_snippet_by_name("breakStatement")')

    def code_next():
        """Inserts next/continue statement"""
        actions.user.insert_snippet_by_name("continueStatement")
        actions.user.deprecate_action("2025-06-24", "user.code_next", 'user.insert_snippet_by_name("continueStatement")')
