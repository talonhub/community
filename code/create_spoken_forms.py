...
from .numbers import digit_string, digits_map, teens, scales, tens
...
# TODO: it'd be nice to be able to say numbers like "2020" as "twenty twenty"
# TODO: Tests
...
def create_spoken_word_for_number(num: int):
...
def create_single_spoken_form(source: str):
    ...
    Returns a spoken form of a string
        ...
        (2) Splits allcaps into separate letters ("ABC" -> a b c d)
...
def create_spoken_forms_from_regex(source: str, pattern: re.Pattern):
    ...
    for piece in pieces:
        ...
        if length > 1 and substring.isnumeric():
            has_fancy_number_version = True
            full_form_fancy_numbers.append(
                create_spoken_word_for_number(int(substring))
            )
